CREATE DATABASE IF NOT EXISTS cct_tpcc;

CREATE TABLE cct_tpcc.public.warehouse (
	w_id INT8 NOT NULL,
...
CREATE TABLE cct_tpcc.public.district (
	d_id INT8 NOT NULL,
	...
	CONSTRAINT district_d_w_id_fkey FOREIGN KEY (d_w_id) REFERENCES cct_tpcc.public.warehouse(w_id) NOT VALID
);

CREATE TABLE cct_tpcc.public.customer (
	c_id INT8 NOT NULL,
	...
	CONSTRAINT customer_c_w_id_c_d_id_fkey FOREIGN KEY (c_w_id, c_d_id) REFERENCES cct_tpcc.public.district(d_w_id, d_id) NOT VALID,
	INDEX customer_idx (c_w_id ASC, c_d_id ASC, c_last ASC, c_first ASC)
);

CREATE TABLE cct_tpcc.public.history (
	...
	CONSTRAINT history_h_c_w_id_h_c_d_id_h_c_id_fkey FOREIGN KEY (h_c_w_id, h_c_d_id, h_c_id) REFERENCES cct_tpcc.public.customer(c_w_id, c_d_id, c_id) NOT VALID,
	CONSTRAINT history_h_w_id_h_d_id_fkey FOREIGN KEY (h_w_id, h_d_id) REFERENCES cct_tpcc.public.district(d_w_id, d_id) NOT VALID
);

CREATE TABLE cct_tpcc.public."order" (
	o_id INT8 NOT NULL,
	...
	CONSTRAINT order_o_w_id_o_d_id_o_c_id_fkey FOREIGN KEY (o_w_id, o_d_id, o_c_id) REFERENCES cct_tpcc.public.customer(c_w_id, c_d_id, c_id) NOT VALID,
	UNIQUE INDEX order_idx (o_w_id ASC, o_d_id ASC, o_c_id ASC, o_id DESC) STORING (o_entry_d, o_carrier_id)
);

CREATE TABLE cct_tpcc.public.new_order (
	...
	CONSTRAINT new_order_no_w_id_no_d_id_no_o_id_fkey FOREIGN KEY (no_w_id, no_d_id, no_o_id) REFERENCES cct_tpcc.public."order"(o_w_id, o_d_id, o_id) NOT VALID
);

CREATE TABLE cct_tpcc.public.item (
	i_id INT8 NOT NULL,
	i_im_id INT8 NULL,
...
CREATE TABLE cct_tpcc.public.stock (
	...
	CONSTRAINT stock_s_w_id_fkey FOREIGN KEY (s_w_id) REFERENCES cct_tpcc.public.warehouse(w_id) NOT VALID,
	CONSTRAINT stock_s_i_id_fkey FOREIGN KEY (s_i_id) REFERENCES cct_tpcc.public.item(i_id) NOT VALID
);

CREATE TABLE cct_tpcc.public.order_line (
	...
	CONSTRAINT order_line_ol_w_id_ol_d_id_ol_o_id_fkey FOREIGN KEY (ol_w_id, ol_d_id, ol_o_id) REFERENCES cct_tpcc.public."order"(o_w_id, o_d_id, o_id) NOT VALID,
	CONSTRAINT order_line_ol_supply_w_id_ol_i_id_fkey FOREIGN KEY (ol_supply_w_id, ol_i_id) REFERENCES cct_tpcc.public.stock(s_w_id, s_i_id) NOT VALID
);
